> Full reference. This is the long-form README that used to live at the repo root; the short version is [`../README.md`](../README.md). Relative links below are from the repo root, so they may need a `../` prefix when viewed from `docs/`.
...
(`GROQ_API_KEY` recommended — free tier; `OPENAI_API_KEY` / `GEMINI_API_KEY`
also work, tried in that order when Groq's key is unset).
